...
if __name__ == '__main__':
    ...
    outdir = carddir + '/deck_' + sys.argv[1]
    ...
    card_faces_filenames = [x for x in os.listdir(carddir)
        if (deckname in x)
        and (x.startswith('card'))
        and (x.endswith('.png'))
        and ('back' not in x)
    ]
    card_faces_filenames.sort()
    card_back_filename = [x for x in os.listdir(carddir)
        if (deckname in x)
        and (x.startswith('card'))
        and (x.endswith('.png'))
        and ('back' in x)
    ...
    for (i,fname) in enumerate(card_faces_filenames):
        i += 1
    ...
    shutil.copy(carddir + '/' + card_back_filename, outdir + '/')
